...
export function AdminHouseProjectsList({ catalogKind }: { catalogKind: HouseProjectCatalogKind }) {
  ...
  const load = useCallback(() => {
    setError("");
    const params = new URLSearchParams({ catalogKind });
    if (search) params.set("search", search);
    fetch(`/api/admin/house-projects?${params.toString()}`)
      .then(async (res) => {
  ...
  async function remove(id: string) {
    if (!confirm("Удалить проект дома?")) return;
    await fetch(`/api/admin/house-projects/${id}`, { method: "DELETE" });
    setProjects((prev) => prev.filter((item) => item.id !== id));
  }

  return (
    <AdminHouseProjectsListView
      catalog={catalog}
      projects={projects}
      search={search}
...
export function AdminHouseProjectsListView({
  catalog,
  projects,
  search,
  onSearchChange,
  ...
  projects: AdminHouseProject[];
  search: string;
  onSearchChange: (value: string) => void;
  loading: boolean;
  error: string;
  onTogglePublished: (project: AdminHouseProject) => void;
  onRemove: (id: string) => void;
}) {
  const editBase = catalog.adminListPath;

  return (
    <div className="space-y-6 max-w-6xl">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">{catalog.adminListTitle}</h1>
          <p className="text-sm text-white/40 mt-1">{catalog.adminListDescription}</p>
        </div>
        <Link
          href={catalog.adminNewPath}
          className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#0F3D2E] hover:bg-[#143f32] text-white text-sm font-semibold transition-colors"
        >
      ...
      {error ? (
        <div className="rounded-xl bg-amber-500/10 border border-amber-500/30 p-4 text-sm text-amber-200">
          <p className="font-semibold">Не удалось прочитать проекты</p>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 rounded-2xl border border-white/[0.08] bg-white/[0.03]">
          ...
          <p className="text-white/45 text-sm">Пока нет проектов в этом каталоге.</p>
        </div>
      ) : (
              ...
              <article key={project.id} className="rounded-2xl overflow-hidden bg-white/[0.03] border border-white/[0.08]">
                <div className="h-40 bg-white/[0.04] relative">
                  ...
                  <div className="absolute left-3 top-3 flex gap-2">
                    {!project.published ? (
                      <span className="rounded-full bg-black/60 px-2 py-1 text-[11px] text-white/60">Черновик</span>
                    ) : null}
                    {project.isNew ? (
                      <span className="rounded-full bg-[#6E2A1F] px-2 py-1 text-[11px] text-white">Новый</span>
                    ) : null}
                  </div>
                </div>
