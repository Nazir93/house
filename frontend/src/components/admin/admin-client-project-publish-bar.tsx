...
type AdminClientProjectPublishBarProps = {
  publishing: boolean;
  hasUnpublishedDraft: boolean;
  onPublish: () => void;
};

/** Плавающая кнопка публикации в ЛК — справа сверху, рядом с переключателем темы. */
export function AdminClientProjectPublishBar({
  publishing,
  hasUnpublishedDraft,
  onPublish,
}: AdminClientProjectPublishBarProps) {
  return (
    <div
      className="admin-client-project-publish-bar fixed z-[68] top-3 right-[4.35rem] sm:top-4 sm:right-[5rem] lg:top-5 lg:right-[5.75rem]"
      aria-label="Публикация в личный кабинет"
    >
      <button
        type="button"
        onClick={onPublish}
      ...
      >
        ...
        <span className="hidden min-[420px]:inline md:hidden">
          {publishing ? "Публикация…" : "Опубликовать в ЛК"}
        </span>
        <span className="hidden md:inline">
          {publishing ? "Публикация…" : "Опубликовать в личном кабинете"}
        </span>
        <span className="min-[420px]:hidden">{publishing ? "…" : "В ЛК"}</span>
      </button>
      {hasUnpublishedDraft && !publishing ? (
        <span
