...
export function AccountShell({
  ...
  supportPhone: string;
  workingHours: string;
  /** Счётчики из БД: платежи в горизонте, открытые тикеты */
  ...
  const { attentionCount, paymentsDue, ticketsActive } = headerSignals;

  const bellHref =
    paymentsDue > 0 ? "/account/payments" : ticketsActive > 0 ? "/account/support" : "/account/dashboard";

  const bellLabel = (() => {
    if (attentionCount === 0) return "Нет напоминаний";
    const parts: string[] = [];
    if (paymentsDue) parts.push(`платежи: ${paymentsDue}`);
    if (ticketsActive) parts.push(`обращения: ${ticketsActive}`);
    return `Напоминания: ${parts.join(", ")}. Перейти`;
  })();
  ...
  return (
    ...
    >
      ...
      <aside
        className={`
          account-premium-sidebar fixed lg:sticky top-0 z-[70] h-screen w-[260px] flex-shrink-0 flex flex-col border-r backdrop-blur-xl
      ...
      >
        <div className="p-4 border-b" style={{ borderColor: "var(--border)" }}>
          <Link
            ...
            className="font-heading font-bold text-lg tracking-tight text-[var(--accent)]"
          >
            {SITE_NAME}
          </Link>
          <p className="text-xs mt-1 uppercase tracking-[0.14em]" style={{ color: "var(--text-muted)" }}>
            Личный кабинет
          </p>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-screen">
        <header
          className="sticky top-0 z-[60] flex items-center justify-between gap-3 px-4 py-3 border-b backdrop-blur-xl supports-[backdrop-filter]:bg-[color-mix(in_srgb,var(--bg)_82%,transparent)]"
          style={{
            borderColor: "var(--border)",
        ...
        >
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <Link
              ...
              style={{
                borderColor: attentionCount > 0 ? "color-mix(in srgb, var(--sale) 45%, var(--border))" : "var(--border)",
                color: attentionCount > 0 ? "var(--sale)" : "var(--text-muted)",
                backgroundColor:
                  attentionCount > 0
                    ? "color-mix(in srgb, var(--sale) 8%, var(--card-bg))"
